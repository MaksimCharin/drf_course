...
def send_telegram_message(chat_id, message):
    ...
    try:
        ...
        logger.info(
            f"Сообщение успешно отправлено в Telegram для chat_id {chat_id}: {message}"
        )
        return response.json()
    except requests.exceptions.RequestException as e:
        logger.error(
            f"Ошибка при отправке сообщения в Telegram для chat_id {chat_id}: {e}"
        )
        return {"error": str(e)}
...
def check_and_send_reminders():
    ...
    for habit in habits:
        ...
        if (
            ...
            and current_time.minute == habit_time.minute
        ):
            ...
            if user_chat_id:
                ...
                if habit.reward:
                    ...
                    message += (
                        ...
                        f"(Приятная привычка: в {habit.linked_habit.time.strftime('%H:%M')} "
                        f"в {habit.linked_habit.place})\n"
                    )

                send_telegram_message.delay(user_chat_id, message)
                logger.info(
                    f"Запланирована отправка напоминания для привычки '{habit.action}' пользователю {habit.user.email}"
                )
            else:
                logger.warning(
                    f"У пользователя {habit.user.email} нет Telegram Chat ID для привычки '{habit.action}'. "
                    f"Напоминание не отправлено."
                )
