...
class UserAPITestCase(APITestCase):
    ...
    def test_user_retrieve_other_forbidden(self):
        ...
        other_user = User.objects.create_user(email='other@example.com', password='other_password')
        response = self.client.get(reverse('users:user_retrieve', kwargs={'pk': other_user.pk}))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_user_update_self(self):
        """Тестирование обновления своего профиля"""
        data = {'city': 'New York'}
        response = self.client.patch(reverse('users:user_update', kwargs={'pk': self.user.pk}), data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_user_update_other_forbidden(self):
        ...
        other_user = User.objects.create_user(email='another@example.com', password='another_password')
        data = {'city': 'London'}
        response = self.client.patch(reverse('users:user_update', kwargs={'pk': other_user.pk}), data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_user_delete_other_forbidden(self):
        ...
        other_user = User.objects.create_user(email='third@example.com', password='third_password')
        initial_user_count = User.objects.count()
        response = self.client.delete(reverse('users:user_delete', kwargs={'pk': other_user.pk}))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class UserTasksTest(APITestCase):
    ...
    def test_block_inactive_users(self, mock_timezone_now):
        ...
        test_current_time = timezone.datetime(2025, 7, 15, 10, 0, 0, tzinfo=timezone.get_current_timezone())

        active_user_login_time = test_current_time - datetime.timedelta(days=10)
        active_user = User.objects.create_user(email='active_task_user@example.com', password='password1')
        active_user.last_login = active_user_login_time
        active_user.save()

        inactive_user_login_time = test_current_time - datetime.timedelta(days=31)
        inactive_user = User.objects.create_user(email='inactive_task_user@example.com', password='password2')
